...
HTTP is the application level protocol that allows web and mobile applications to communicate over the internet. It works using a client–server model, where the client requests a resource and the server responds with the requested data. The term _hypertext_ refers to text that contains links to other resources and is easy for users to navigate.
