...
```bash
SubhaDip03@htb[/htb]$ curl http://SERVER_IP:PORT/serial.php -X POST -d "param1=sample"
...
```bash
SubhaDip03@htb[/htb]$ echo https://www.hackthebox.eu/ | base64
...
```bash
SubhaDip03@htb[/htb]$ echo aHR0cHM6Ly93d3cuaGFja3RoZWJveC5ldS8K | base64 -d
...
```bash
SubhaDip03@htb[/htb]$ echo https://www.hackthebox.eu/ | xxd -p
...
```bash
SubhaDip03@htb[/htb]$ echo 68747470733a2f2f7777772e6861636b746865626f782e65752f0a | xxd -p -r
...
### Caesar/Rot13 <a href="#caesarrot13" id="caesarrot13"></a>
...
Even though this encoding method makes any text looks random, it is still possible to spot it because each character is mapped to a specific character. For example, in <mark style="color:$info;">`rot13`</mark>, <mark style="color:$info;">`http://www`</mark> becomes <mark style="color:$info;">`uggc://jjj`</mark>, which still holds some resemblances and may be recognized as such.
...
```bash
SubhaDip03@htb[/htb]$ echo https://www.hackthebox.eu/ | tr 'A-Za-z' 'N-ZA-Mn-za-m'
...
```bash
SubhaDip03@htb[/htb]$ echo uggcf://jjj.unpxgurobk.rh/ | tr 'A-Za-z' 'N-ZA-Mn-za-m'
...
### Other Types of Encoding <a href="#other-types-of-encoding" id="other-types-of-encoding"></a>
...
<mark style="color:$info;">`If you face any similar types of encoding, first try to determine the type of encoding, and then look for online tools to decode it.`</mark>
