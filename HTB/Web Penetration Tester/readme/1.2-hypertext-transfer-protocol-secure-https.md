...
In the previous section, we discussed how HTTP requests are sent and processed. HTTPS was introduced to fix a major weakness of HTTP, where data is sent in plain text and can be read by anyone intercepting the traffic. With HTTP, attackers in the middle of the connection can easily view or modify the data being transferred. HTTPS solves this by encrypting all communication between the user and the server, making the data unreadable even if it is intercepted. Because of this added security, HTTPS is now the standard for modern websites, and HTTP is gradually being deprecated by browsers.
...
```bash
SubhaDip03@htb[/htb]$ curl https://inlanefreight.com
...
```bash
SubhaDip03@htb[/htb]$ curl -k https://www.inlanefreight.com
