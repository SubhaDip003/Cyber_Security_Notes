...
```bash
curl http://83.136.249.34:36245/api.php/city/london
```
...
```bash
SubhaDip03@htb[/htb]$ curl -s http://<SERVER_IP>:<PORT>/api.php/city/london | jq
...
```bash
SubhaDip03@htb[/htb]$ curl -s http://<SERVER_IP>:<PORT>/api.php/city/le | jq
...
```bash
SubhaDip03@htb[/htb]$ curl -s http://<SERVER_IP>:<PORT>/api.php/city/ | jq
...
```bash
curl -X POST http://83.136.249.34:36245/api.php/city/ -d '{"city_name":"Kolkata", "country_name":"India"}' -H 'Content-Type: application/json'
```
...
```bash
curl -X PUT http://83.136.249.34:36245/api.php/city/Kolkata -d '{"city_name":"Delhi", "country_name":"India"}' -H 'Content-Type: application/jso
n'
```

We see in the example above that we first specified `/city/Kolkata` as our city, and passed a JSON string that contained `"city_name":"Delhi"` in the request data. So, the london city should no longer exist, and a new city with the name `Delhi` should exist. Let's try reading both to confirm:

```bash
curl http://83.136.249.34:36245/api.php/city/Delhi
[{"city_name":"Delhi","country_name":"India"}]
```
...
> Note: In some APIs, the `Update` operation may be used to create new entries as well. Basically, we would send our data, and if it does not exist, it would create it. For example, in the above example, even if an entry with a `Kolkata` city did not exist, it would create a new entry with the details we passed. In our example, however, this is not the case. Try to update a non-existing city and see what you would get.
...
```bash
curl -X DELETE http://83.136.249.34:36245/api.php/city/Delhi
```

<figure><img src="../.gitbook/assets/image (94).png" alt=""><figcaption></figcaption></figure>

As we can see, after we deleted `Delhi`, we get an empty array when we try reading it, meaning it no longer exists.
